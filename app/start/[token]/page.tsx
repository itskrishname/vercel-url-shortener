...
async function getLink(token: string) {
  await dbConnect();
  const link = await LinkModel.findOne({ token: token });
  if (!link) return null;
...
export default async function StartPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  // We render a client-side component or script that handles the redirect.
  // This allows showing ads, a timer, or security checks.
  // For now, we'll implement a simple timer redirect.
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 font-sans">
      ...
      <RedirectLogic url={link.originalUrl} />
    </div>
  );
}
        ...
        <script
            dangerouslySetInnerHTML={{
                __html: `
                    ...
                    }, 2000); // 2 second delay for "protection"/UX
                `
            }}
        />
    )
